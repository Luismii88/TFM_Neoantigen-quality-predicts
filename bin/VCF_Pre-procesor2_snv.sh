...
raw_files=$(ls $p_dir | grep -v 'tbi' | grep 'snv')
# Encuentra los archivos VCF de variantes de un solo nucleotido (snv) y excluye los archivos .tbi.
...
do
    ...
    cp "$p_dir/$file" "$p2_dir/${output_vcf}.vcf.gz"
    # Copia el archivo SNV al directorio de salida sin concatenar.

    echo "VCF files procesados"
    # Muestra un mensaje indicando que los archivos VCF han sido procesados.
...
do
    ...
    ~/miniforge3/envs/neo/bin/vep -i "$p2_dir/${file}.vcf.gz" --cache --force --offline --vcf --compress_output gzip --fork 4 --protein -o "$a_dir/${file}_ann.vcf.gz" --sf $a_dir/${file}_ann.html --fa ${m_dir}/GRCh38_full_analysis_set_plus_decoy_hla.fa --gtf ${m_dir}/gencode.v45.annotation.gtf.gz --force_overwrite --terms SO --tsl --biotype --hgvs --plugin Frameshift --plugin Wildtype
    # Ejecuta la anotación VEP en los archivos VCF usando referencias de genoma y diferentes plugins para variantes de genes.
    # En vant es ~/miniforge3/envs/neo/bin/vep y en labo es ~/ensembl-vep/vep
    ...
    ~/miniforge3/envs/neo/bin/filter_vep -i $a_dir/${file}_ann.vcf.gz -f "(SYMBOL != $genes_to_remove) and (BIOTYPE is protein_coding)" --format vcf --gz -o "$a_dir/${file}_ann_fil.vcf"
